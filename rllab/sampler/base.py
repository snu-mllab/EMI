...
class BaseSampler(Sampler):
    ...
    def process_samples(self, itr, paths):
        ...
        logger.record_tabular('StdRawReturn', np.std(raw_returns))
        logger.record_tabular('MaxRawReturn', np.max(raw_returns))
        logger.record_tabular('MinRawReturn', np.min(raw_returns))

        if ("bonus_rewards" in paths[0]):
            bonuses = np.concatenate([path["bonus_rewards"] for path in paths])
            logger.record_tabular('StdBonusReturn', np.std(bonuses))
            logger.record_tabular('MaxBonusReturn', np.max(bonuses))
            logger.record_tabular('MinBonusReturn', np.min(bonuses))
            logger.record_tabular('AverageBonusReturn', np.mean(bonuses))
